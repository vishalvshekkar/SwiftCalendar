...
struct DateStructure {
    ...
    func getUTCNSDate() -> NSTimeInterval {
        let formatter = NSDateFormatter()
        formatter.dateFormat = "yyyy-MM-dd"
        return formatter.dateFromString("\(self.year)-\(self.month)-\(self.day)")!.timeIntervalSince1970 * 1000
    }
    
    func getUTCEndNSDate() -> NSTimeInterval {
        let formatter = NSDateFormatter()
        formatter.dateFormat = "yyyy-MM-dd HH:mm:ss"
        return formatter.dateFromString("\(self.year)-\(self.month)-\(self.day) 23:59:59")!.timeIntervalSince1970 * 1000
    }
    
    func getTZOffset() -> Double {
        let localFormatter = NSDateFormatter()
        localFormatter.dateFormat = "yyyy-MM-dd"
        localFormatter.timeZone = NSTimeZone.localTimeZone()
        let offSet = -1*localFormatter.dateFromString("1970-01-01")!.timeIntervalSince1970
        return offSet
    }
...
let defaultFontForCalendar = UIFont.systemFontOfSize(12)
